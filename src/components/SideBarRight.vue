<template>
    <div class="h-full flex flex-col bg-sky-50 shadow-sm">
        ...
        <div class="h-16 flex flex-row justify-center items-center gap-1">
            <button class="btn btn-sm btn-outline btn-info" @click="loadBoneData('RUS-CHN')">RUS-CHN</button>
            <button class="btn btn-sm btn-outline btn-info" @click="loadBoneData('TW3-C-RUS')">TW3-C-RUS</button>
            <button class="btn btn-sm btn-outline btn-info" @click="loadBoneData('TW3-C-CARPAL')">TW3-C-CARPAL</button>
        </div>
        <!-- 信息输入区域 -->
...
export default {
    ...
    data() {
        return {
            boneData: {
                'RUS-CHN': ['桡骨', '尺骨', '第一掌骨', '第三掌骨', '第五掌骨', '第一近节指骨', '第三近节指骨', '第五近节指骨', '第三中节指骨', '第五中节指骨', '第一远节指骨', '第三远节指骨', '第五远节指骨'].map((name) => ({ name, score: '待计算' })),
                'TW3-C-RUS': ['骨头一', '桡骨', '尺骨', '第一掌骨', '第三掌骨', '第五掌骨', '第一近节指骨', '第三近节指骨', '第五近节指骨', '第三中节指骨', '第五中节指骨', '第一远节指骨', '第三远节指骨', '第五远节指骨'].map((name) => ({ name, score: '待计算' })),
                'TW3-C-CARPAL': ['头状骨', '钩骨', '三角骨', '月骨', '舟骨', '大多角骨', '小多角骨'].map((name) => ({ name, score: '待计算' }))
            },
            currentBoneData: []
        };
    },
